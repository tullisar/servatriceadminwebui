<?php
	...
	if (trim(strtolower($enabled)) == "yes"){
		...
		$query = mysqli_query("SELECT * FROM " . $uptimetable . " WHERE timest LIKE '" . $currentdate . "%' AND id_server = " . $serverid);
		while ($row = mysqli_fetch_array($query)){
			echo '[\'' . date_format(date_create($row['timest']), 'h:i:s A') . '\',' . ($row['tx_bytes'] / $rxtxratecalc) . ',' . ($row['rx_bytes'] / $rxtxratecalc) . '],';
		}

		$query = mysqli_query("SELECT MAX(timest) FROM " . $uptimetable . " WHERE timest LIKE '" . $currentdate . " " . $currenthour . "%'");
